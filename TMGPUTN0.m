TMGPUTN0 ;TMG/kst/TIU Document Upload look-up function ;03/25/06; 3/24/21, 9/30/25
	...
	;
	;"-----------------------------------------------------------------------------------------------
	;"LOOKUP(DocTitle,Autosign) -- Upload look-up function
	;"PtArrayCreate(Array) --To put global scope vars (i.e. TMGNAME,TMGSSNUM etc) into n array for easier portability"
	;"DocArrayCreate(Document) -- put TIUVDT etc. etc into an array for easier portibility
	;"PrepDoc(Document,NewDoc) -- return a blank document
	;"GetDocTIEN(Title) -- return IEN for document *type defination* / Identify document title
	;"GetLocIEN(Location) -- return IEN for location
	;"GetService(IEN) -- Get the Service for the Provider
	;"GetProvIEN(Provider) -- return IEN for Provider
	;"GetRecord(Document,NewDoc,AskOK,Editable) -- To get a record--either via creating a NEW one, or returning an existing one (first emptied)
	;"DocExists(Document) -- To return document IEN, IF it  already EXISTS for the given patient, title, and visit.;
	;"BuffCharCount() -- count the number of characters in the current upload buffer, for the current document
	;"PrepUploadBuf() -- Ensure upload buffer is ready for processing
	;"NeedsReformat(MaxWidth) -- scan the single note being processed, to see if it is too wide
	;"CutNote(Array) -- extract the current note out of the entire upload buffer
	;"PasteNote(Array,NextNoteI) -- put Array back into the upload buffer, at the correct location,
	;"CompToBuff(ExistingIEN,UplTIEN,UplDate) -- compare the document being uploaded (i.e. in the file 8925.2, TIU upload buffer) to documents already existing in database
	;"CreateRec(Document) -- Create document record - Returns DA
	;"StuffRec(Document,PARENT) -- Stuff fixed field data
	;"MakeVisit(Document) -- create a NEW entery in the VISIT file, based on info in Document.;
	;"FOLLOWUP(DocIEN) ;Post-filing code for PROGRESS NOTES
	;"Set8925Value(TMGFDA,Value,Field,IsIEN) -- provide a clean means of loading values into fields, into TMGFDA(8925,DOCIEN)
	;"FOLLOWUP(DocIEN) ;" Post-filing code for PROGRESS NOTES
	;"REFILE -- trigger a retry of filing the TIU UPLOAD BUFFER
	;"-----------------------------------------------------------------------------------
	;"-----------------------------------------------------------------------------------
	;
...
GetProvIEN(Provider)
	...
	IF +Provider=Provider QUIT Provider  ;"If numerical input sent in, assume it is DUZ and return it
	;
	NEW DIC,X,Y
