...
FETCHTIU
 ...
 FOR  SET TIUIEN=$O(^TIUX(8925,TIUIEN)) QUIT:(TIUIEN'>0)!(ABORT=1)  DO
 . NEW TMGDFN,TMGDUZ,TEXTARR,BDT,TITLEIEN
 . NEW ZN SET ZN=$G(^TIUX(8925,TIUIEN,0))
 . SET TITLEIEN=$P(ZN,"^",1),TMGDFN=$P(ZN,"^",2)
 . SET BDT=$P(ZN,"^",7)
 . SET TMGDUZ=$P($G(^TIUX(8925,TIUIEN,12)),"^",2)
 . MERGE TEXTARR("TEXT")=^TIUX(8925,TIUIEN,"TEXT")
 . WRITE "==NOTE TEXT==",!
 . ZWR TEXTARR
 . NEW % WRITE "IMPORT NOTE" DO YN^DICN
 . IF %'=1 QUIT
 . ;"WRITE "CONTINUING",!
 . NEW NOTETITLE,USERNAME,ONETIU
 . SET NOTETITLE=$P($G(^TIU(8925.1,TITLEIEN,0)),"^",1)
 . SET USERNAME=$P($G(^VA(200,TMGDUZ,0)),"^",1)
 . WRITE "I AM IMPORTING THIS TO: ",$P($G(^DPT(TMGDFN,0)),"^",1),!
 . WRITE "THIS IS FOR USER ",USERNAME,!
 . DO BLANKTIU^TMGRPC1(.ONETIU,TMGDFN,USERNAME,6,BDT,NOTETITLE)
 . ; NEW ALRTRESULT
 . ; DO INFRMALT^TMGXQAL(.ALRTRESULT,150,"ERROR CREATING BLANK NOTE FOR: "_TMGDFN)
 . NEW IDX SET IDX=1
 . ;" SET NOTETEXT("TEXT",IDX,0)
 . SET TEXTARR("HDR")="1^1"
 . ;
 . NEW RESULT DO SETTEXT^TMGTIUS1(.RESULT,ONETIU,.TEXTARR,0)
 . DO SEND^TIUALRT(TIUIEN)
 . WRITE !,!,"ABORT" DO YN^DICN
 . SET ABORT=%
 QUIT
TEST
 NEW TIUIEN SET TIUIEN=0
