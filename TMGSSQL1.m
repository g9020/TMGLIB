...
PAFSDUE()   ;"SQL Statement for getting all missing BC/BS PAFs for this year
 QUIT "SELECT LAST_NAME, FIRST_NAME, CPT_CODE, VISIT_DATE_FROM, DOB, ACCOUNT_NUM, PATIENT_SEQ_NUM FROM (SELECT pd.LAST_NAME, pd.FIRST_NAME, pd.DOB, pd.ACCOUNT_NUM, pd.PATIENT_SEQ_NUM, uc.CPT_CODE, uc.VISIT_DATE_FROM, ROW_NUMBER() OVER (PARTITION BY pd.PATIENT_SEQ_NUM ORDER BY uc.VISIT_DATE_FROM DESC) AS rn FROM U_CHARGES uc JOIN U_PATIENT_DEMOGRAPHIC pd ON uc.PATIENT_SEQ_NUM = pd.PATIENT_SEQ_NUM JOIN U_PATIENT_PLANS pp ON pd.PATIENT_SEQ_NUM = pp.PATIENT_SEQ_NUM JOIN U_PLAN p ON pp.PLAN_SEQ_NUM = p.PLAN_SEQ_NUM WHERE uc.VISIT_DATE_FROM > TO_DATE('2026-01-01', 'YYYY-MM-DD') AND uc.CPT_CODE LIKE '99%' AND p.PLAN LIKE '%BCBS_A%' AND pp.ACTIVE_PLAN = 'Y' AND NOT EXISTS (SELECT 1 FROM U_CHARGES uc2 WHERE uc2.PATIENT_SEQ_NUM = uc.PATIENT_SEQ_NUM AND uc2.VISIT_DATE_FROM > TO_DATE('2026-01-01', 'YYYY-MM-DD') AND uc2.CPT_CODE = '96161')) WHERE rn = 1 ORDER BY LAST_NAME, FIRST_NAME"
 ;"
...
SQLREQUEST(SQLSTATEMENT,SQLOUTPATH,SQLOUTNAME,DEBUGMODE,LOGFILE)  ;"
 ...
 . FOR I=1:1:6 DO  QUIT:DONE=1   ;"This will cut the number of checks down to 6 or 30 seconds. If it isn't done in 30 seconds something is probably wrong.    2/16/26
 . . IF DEBUGMODE=1 WRITE "."
